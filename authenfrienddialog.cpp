    ...
    , ui(new Ui::AuthenFriendDialog)
{
    ...
    // 连接展示更多标签事件
    connect(ui->more_lb, &ClickedOneceLabel::clicked, this, &AuthenFriendDialog::ShowMoreLabel);

    // 连接输入标签回车事件
    connect(ui->lb_edit, &CustomizedEdit::returnPressed, this, &AuthenFriendDialog::SlotLabelEnter);
    connect(ui->lb_edit, &CustomizedEdit::textChanged, this, &AuthenFriendDialog::SlotLabelTextChange);
    connect(ui->lb_edit, &CustomizedEdit::editingFinished, this, &AuthenFriendDialog::SlotLabelEditFinished);
    connect(ui->tip_lb, &ClickedOneceLabel::clicked, this, &AuthenFriendDialog::SlotAddFirendLabelByClickTip);
...
void AuthenFriendDialog::InitTipLbs()
{
    ...
    for (size_t i = 0; i < _tip_data.size(); ++i)
    {
        ...
        //
        connect(lb, &ClickedLabel::clicked, this, &AuthenFriendDialog::SlotChangeFriendLabelByTip);
...
void AuthenFriendDialog::AddTipLbs(ClickedLabel* lb, QPoint cur_pt, QPoint &next_pt, int text_width, int text_height)
{
    lb->move(cur_pt);
    lb->show();
...
void AuthenFriendDialog::SetApplyInfo(std::shared_ptr<ApplyInfo> apply_info)
{
    ...
    ui->back_edit->setPlaceholderText(_apply_info->name);
}
...
void AuthenFriendDialog::AddLabel(QString name)
{
    ...
    connect(tmp_label, &FriendLabel::sig_close, this, &AuthenFriendDialog::SlotRemoveFriendLabel);
...
void AuthenFriendDialog::SlotChangeFriendLabelByTip(QString lbtext, ClickLbState state)
{
...
/**
 * @brief AuthenFriendDialog::SlotApplySure     receiver同意添加为好友按钮（同意添加好友通知）
 */
void AuthenFriendDialog::SlotApplySure()
{
    qDebug()<<"Slot Auth Sure called" ;
    // 发送同意添加好友请求的逻辑（Authen）
    QJsonObject json_obj;
    json_obj["fromuid"] = UserMgr::GetInstance()->GetUID();
    json_obj["touid"] = _apply_info->uid;
